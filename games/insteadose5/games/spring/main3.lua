...
room {nam = 'main';
	title = 'Вступление';
	decor = 'Вместо музыки в игре используются фоновые звуки и шумы, поэтому для создания атмосферы рекомендуется включить звук в настройках.',
	obj = { obj 
			{dsc = '{Начать}',
					act = function(s)
							walk 'прихожая'
...
room { 	nam = 'прихожая';
	...
	nam = '#полка';
           ...
           obj{ 
	nam = '#солнце';
				act = 'Начало лета. Вчера был дождь, а сегодня на небе ни облачка.'
				};
           obj{ 
	nam = '#Аня';
				dsc = function(s)
...
room { 	nam = 'двор';
	title = 'Двор';
	decor = 'Двор залит ярким, уже совсем летним солнцем. Листья и трава еще совсем молодые, и от того светятся особым ярко-зеленым светом, вчерашние лужи почти высохли, надо бы {#погулять|погулять} сегодня вечером. Рядом с подъездом припаркована {моя_машина|моя машина}. ';
...
dlg { 	nam = 'выбор_платья';
	...
	phr = {  only = true;
			...
			{'Аня, мы опоздаем!','Аня разводит руками:^ -- Конечно опоздаем, ведь мне абсолютно нечего надеть!',next ='#выбор'},
			{ hidden = true, '#выбор',
				only = true;
...
obj { nam = 'радио';
	fm = 0;
	act = function(s) 
				local fs = {'Я включаю магнитолу и выбираю станцию. Так, тут обычно играет разный шансон...','Я выбираю следующую станцию. Тут какой-то мужик рассказывает про кризис: "Если сейчас просто исчезнут все те акции, которые у нас ходят на фондовом рынке, с промышленностью и с другими предприятиями в стране ничего страшного не случится..."','Я выбираю следующую станцию, тут просто ритмичная электронная музыка без слов.','Я выключил магнитолу. Надоело.'};	
				s.fm = s.fm + 1
...
room {	nam = 'Машина4',
	...
	decor = [[Машина подскочила на кочке и я проснулся. В голове осталось тягостное воспоминание от дурного {#сон|сна}. Осенний дождь бьет в лобовое стекло. Слева от меня хмурый {#водитель|водитель}. Уже темнеет, судя по придорожным кафешкам и рекламным щитам, мы подъезжаем к городу.]];
	obj = { 
			obj {nam = '#сон',
...
obj {	nam = 'ключ_от_номера',
	...
	use = function(s,w)
		...
		else p 'Не понимаю, как мне поможет ключ.'
		end;
	end;
}
...
dlg { nam = 'водитель';
	title = 'Водитель';
	dsc = 'Это большой молчаливый мужик в кожаной куртке. Если бы я не знал, что он из ФСБ, то принял бы за какого-нибудь бандита из сериала. Он представился Романом и почти всю дорогу молчал. ';
	phr = {
...
wrd = function(l)
	...
	o.act = 'Выберите вариант перевода и замените им значение в [?]';
...
function trns_stat_str(roomnam)
	...
	--elseif res >= cnt - 2 then return 'Прогресс перевода: почти закончен.'
...
protocol { 	nam = 'идея_0';
	...
	decor =  'У этого языка очень нетипичная типология, предложения начинаются со сказуемого, затем идет прямое дополнение, а уже за ним подлежащее. Получается "мыла раму мама", нехарактерно ни для афразийской ветви, ни для шумерского.';
	obj = {     
     		};
}
...
protocol { 	nam = 'идея_1';
	...
	decor =  'Надпись на диске выполнена в виде сходящейся спирали: goo hara-gamkra vahita . hanaha vahita kuksata manahata hara krahani tah tarjan . kir durjala vahita famhata . jags tarta vahita takja tur hara-gamkra vahita . pakra krahani dusur durjala gamkra';
	obj = {     
		
				lblc('Любая расшифровка начинается с подсчета частоты слов. Например в церемониальных текстах самое частое слово будет "царь" или "бог". В нашем случае чаще всего встречается слово'),
				wrd('vahita'); 
...
protocol { 	nam = 'идея_4';
	...
	decor =  'Надпись на диске выполнена в виде сходящейся спирали: goo hara-gamkra vahita . hanaha vahita kuksata manahata hara krahani tah tarjan . kir durjala vahita famhata . jags tarta vahita takja tur hara-gamkra vahita . pakra krahani dusur durjala gamkra';
	obj = {     
				lblc('Три слова вызывают у меня наибольшие сомнения. Они не встречаются в других текстах, остается только положиться на интуицию ученого и чувство языка...'),
				wrd('durjala'); 
				wrd('tur');
				wrd('dusur');				
	...
	phr = {
			...
			{ hidden = true, '#подделка',
				...
				{'В тексте угадываются отдельные слова, но написаны они безграмотно.','-- Значит, хоть что-то вы все-таки можете прочитать? -- генерал как будто обрадовался.',
					only = true,
					{'Могу, но это бессмыслица. Просто чья-то мистификация...','-- Значит, вы уверены, что мистификация?'},
			...
			{ hidden = true, '#почему_подделка',
				{'Я уверен, что это подделка. Причем сделана уже после того, как Жан-Франсуа Шампольон расшифровал египетские иероглифы, и после того, как Георг Фридрих Гротефенд проник в тайны шумерской клинописи. То есть этот предмет изготовили не ранее 19 века.','-- Прекрасно. Я рад, что академик Неймарк порекомендовал именно вас!',
...
dlg { 	nam = 'задача';
	...
	phr = {
			...
			{ hidden = true, '#подходите', '',
				...
				{'Но надписи... Это же искусственный язык!','-- Профессор Шевченко тоже так думал, а в итоге пришел к другому выводу.^ Генерал взял листочек и зачитал:^ -- "Неизвестный ранее язык, близкий к праностратическому. Возможный предок как для афразийской так и для индоевропейской языковой семьи."',
...
dlg { 	nam = 'задача2';
	...
	phr = {
			{'Ну так.','-- Отлично, давайте зайдем к вам, переговорим.^ Он зашел ко мне в кабинет и закрыл дверь.^ -- Вы пообщались с Петром Геннадьевичем? Он вам рассказал о природе объекта?',
				only = true,			
				{'Да, пообщались.','-- Тогда вы уже знаете, что этот диск вроде рации. Так вот, я хочу, чтобы вы поговорили с ними. На языке, который вы расшифровали.',next = '#дальше'},
			...
			{ hidden = true, '#дальше',
				...
				onempty = function()
					p 'Генерал многозначительно посмотрел на меня и вышел в коридор. Я потер виски -- интересно, сколько сейчас времени. Очень устал, хочется спать.'
					walkout()
				end;
		}
...
dlg { 	nam = 'задача3';
	...
	phr = { 		
			...
			{ hidden = true, '#что2', function() p ('-- Понимаете, там есть одно очень сложное для перевода место. Я не уверен, но если дословно, то в нем прячется "'..dict['tur'][2]..'". Еще там сказано: "'..dict['pakra'][2]..' '..dict['krahani'][2]..' '..dict['dusur'][2]..' '..dict['durjala'][2]..' '..dict['gamkra'][2]..'", -- генерал нахмурился. -- Белиберда какая-то. А если по-русски?') end,
					only = true,
					...
					{'Я думаю эксперименты дадут человечеству импульс в будущее. Их нужно продолжить.','-- Я в этом не сомневался, Алексей Аркадьевич! Думаю, пора готовиться к первому контакту.',next = '#что'},
					onempty = function()
	...
	obj = { 
			...
			obj { nam = 'подстаканник';
				dsc = function(s) 
							if progress == 0 then 
								return true 
							else return 'Перед генералом стоит еще один стакан в красивом {подстаканнике}, из которого он периодически отхлебывает дымящийся чай.' 
							end;
						end;
				act = function(s) 
				  ...
				  inv = function(s)
					...
					use = function(s,w)
								...
								elseif w^'раковина' then
									p 'Наполнить сырой водой? Лучше в чайнике вскипятить.'
...
obj {	nam = 'чайник';
	...
	act = function(s)
			if where(s)^'кабинет' and (not s.hot) and s.full and s.connect then 
				...
				if not have('чай') then
					return 'Можно было бы его вскипятить, да только все-равно чая нет. Может завтра куплю, если не забуду.'
				else
					s.hot = true
	...
	use = function(s,w)
				...
				elseif w^'стакан' and not s.hot then	
					p 'Сначала чайник нужно вскипятить.'
...
room {	nam = 'кабинет';
	...
	decor = 'Я в небольшом кабинете с желтыми стенами и старым линолеумом. Тут стоят два коричневых {#стол|стола}, высокий {#шкаф|шкаф} для одежды, на стене висит {доска|доска}. Противоположную стену занимают {#стеллажи|стеллажи} с книгами и папками. Напротив двери большое {#окно|окно}.';
	onexit = function(s,w)
	...
	obj = {	obj { nam = '#стол';
			...
			obj {	nam = 'доска';
				act = 'Обычная доска, на которой можно писать мелом.'; 
				};	
			obj {	nam = '#стеллажи';
				act = function(s) 
						if idea2 == 1 then 
...
dlg { nam = 'д_охранник';
	...
	phr = {
			{'Здравствуйте.','Охранник смотрит на меня и машинально кивает:^ --Здравствуйте.'},
			onempty = function()
					walkout()
				end;
...
room {  nam = 'улица',
		...
		obj = {    'голуби',
					...
					obj {nam = '#строения',
							act = 'Пара одноэтажных зданий, может быть трансформаторная будка и какой-нибудь склад.'
							},	
					obj {nam = '#гостиница',
		...
		decor = function(s)
				  ...
				  elseif (progress > 1) and  (idea4 ~= 2) and (trns_flag('протокол_4') or trns_flag('протокол_5') )  then
				   ...
				   local mes = {'Я отправился в гостиницу и забылся беспокойным сном. Меня то и дело будили звуки из номера сверху. Не знаю, что там происходило, и не хочу знать. Мне снилось, что я еду на машине и не могу вспомнить, куда именно. Как будто это очень важно, как будто от этого зависит моя жизнь. Я проснулся с больной головой и отправился в институт.',
					...
					'Я добрался до номера в каком-то забытьи, даже не помню, как уснул. Мне снилась Аня, она стояла у моей кровати и что-то говорила, а я не слышал, хотел ответить ей, но не мог, как будто все тело было ватное, и вокруг все заполнено ватой. Тяжелой, давящей на виски ватой. Я проснулся в поту и долго не мог снова уснуть. Утро не принесло ничего необычного - та же мерзкая погода, тот же институт.'}
				   p( mes[r])
...
dlg { 	nam = 'объяснение';
	...
	dsc = '-- А вы наш новый лингвист! Приятно познакомиться! -- он потряс мою руку и тут же отошел на шаг назад, как будто я собираюсь его схватить. Все физики немного странные.';
	phr = {
			...
			{ hidden = true, '#вывод','Один из приборов начал пищать, и Петр Геннадьевич смущенно покосился на него.^ -- Если резюмировать все сказанное - мы сами до конца не понимаем, с чем столкнулись. Похоже, все наши представления о времени неверны и находятся в зачаточном состоянии.',
					{'Вы хотите сказать, что можно отправиться в прошлое? И поменять в нем что-то?','-- Не совсем. Портал открывается не в прошлое, а в другой вариант этого прошлого. Мы пока до конца не понимаем всех нюансов, но это величайшее открытие! Его даже не с чем сравнить.'},
...
room { nam = 'лаборатория',
		...
		title = 'Лаборатория',
		decor = function(s)
			      return 'Это просторное помещение с несколькими большими столами, на которых выставлены физические {#приборы|приборы}. За дальним столом {#дальний|мужчина} в халате возится с какими-то микросхемами. Ближе ко входу еще один, {#ближний|физик} увлеченно рассматривает экран осциллографа. На стене висит {#доска|доска}, исчерченная мелом.'
			end;
		obj = {  obj {nam='#приборы', act = 'Они меня всегда завораживали.'},
				 obj {nam='#доска', act = 'Пара графиков и множество непонятных формул, сделанных разным почерком. На доске я замечаю {#мел|коробочку} с мелом.'},
				 obj {	nam='#дальний',
						isMan = true;
						act = function(s) 
...
dlg { nam = 'автостоп';
	title = 'Водитель';
	dsc = 'Я поднял руку, и вишневая "Нива" остановилась прямо напротив. Из окна выглянул усатый водитель, он таращится на меня как на привидение.';
	phr = {
			...
			{hidden = true, '#что','Я, как мог, отряхнул снег с одежды и забрался на переднее сиденье. Водитель тут же включил посильнее печку и поехал.',
				...
				{'Меня похитили. Бандиты какие-то.','-- Ничего себе! Денег хотели? Коммерсант, что ли?',
					only = true,
			...
			onempty = function()
					p 'Меньше чем через час мы были на месте. Я попрощался с Сашей -- так его звали -- поблагодарил и поднялся на пятый этаж.'
					walk('перед_дверью')
				end;
		};
}
...
dlg { nam = 'аня';
	...
	phr = {
			...
			onempty = function()
					p 'Я зашел в коридор, разулся и прошел на кухню. Тут почти ничего не изменилось, мы много раз засиживались тут допоздна, когда еще Аня не жила у меня.'
					walk('кухня')
				end;
		};
}
...
dlg { nam = 'аня_кофе';
	...
	phr = {
			{'Спасибо, вкусный кофе.','--Ты сам говорил, 15 минут.',
				{'Конечно, не буду терять время, я хочу начать все сначала.','-- А ты не думаешь, что за год я могла встретить кого-нибудь?', 
					...
					{'Год? Авария была этим летом.','-- Ты все перепутал. Тебя так и не вылечили после неё!',
						{'Вылечили от чего?','-- Ты ударился головой, тебя еле спасли.'},
...
room { nam = 'зима',
		title = 'Зима',
	   decor = 'На помойке рядом с домом я нашел старую драную куртку и захватил с собой, чтобы не замерзнуть. Почти три часа я добирался на попутках к нужному километровому столбику. Пока мы ехали, большие беззвучные хлопья снега медленно падали с неба. Я осмотрелся и пошел в лес по собственным, почти засыпанным снегом следам. Я шел, до рези в глазах всматриваясь в просветы между деревьями, надеясь заметить блеск портала. Снег все шел, как-то внезапно стемнело. Я выбился из сил и присел отдохнуть. Холод пробирался под одежду. Я закрыл глаза и задремал. Последнее что я помню перед тем, как уснуть, это голос генерала Иволгина: "Вот он, в сугробе! Забирайте, пока портал держится!". Или все это мне только показалось?^^'..fmt.c("КОНЕЦ^^")..fmt.c('Михаил Поздняков (casper_nn), 2017^')..'^^'..fmt.c("{@restart|ВЕРНУТЬСЯ К ЖУРНАЛУ}")
