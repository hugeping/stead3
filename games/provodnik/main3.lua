...
dlg {
	...
	enter = function(s)
		if not visited() then
			p [[Дверь открыл мужчина в смокинге. Он выглядел так, как будто собирался в дорогу. В его руке был чемодан.]]
		else
...
завершает картину.]];
		...
		}:with
		{
			...
			obj {
				nam = '#шкафчик';
				act = function() p [[Тут только парфюмерия.]]; enable 'парфюм' end;
				obj = {
					obj {
						nam = 'парфюм';
...
красной скатертью, которая, впрочем, едва заметна за обилием {#еда|еды и выпивки}.]];
		if transport then
...
guard = obj {
	...
	dsc = function(s)
		if live(s) then
			...
			p [[Я вижу перед собой {человека}, который, скорчившись, держится за свое левое плечо.]]
		end
	end;
	used = function(s, w)
		if w^'шланг' then
...
Тварь смотрела на меня своими маленькими глазами, и мне казалось, что моей души коснулся весь жар и холод преисподней.]],
