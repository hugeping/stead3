...
room {
	...
	enter = function()
		...
		p [[12 лет сигнал с Земли добирался сюда. Но для меня время замкнулось. Время отправки сообщения с Земли стало "здесь и сейчас" на "Пилигримме". Иначе просто можно сойти с ума.]]
	end;
}: with
{
	...
	obj {
		...
		act = function(s)
			local txt = {
				...
				[[-- Сынок, здравствуй! Не беспокойся о нас! У нас все хорошо. Главное, как ты там, один?... ]],
				[[-- Да, я помню, что твой ответ придет только... Ах, ты просил делать вид, что расстояния между нами не существует. Ты у меня такой умный! Ведь так все и есть!]],
				...
				[[-- Я уже смирилась с тем, что не увижу тебя здесь. Я знаю, что у тебя не было выбора. Ведь ты такой умный и должен был лететь ради всех нас, ради наших детей... Так что не вини себя. Да и что будет с нами, сейчас никто не знает.]],
				[[-- Ох, я опять все испортила! В общем, просто подумай о нас, а мы с папой будем всегда с тобой!]],
...
уточнять это у Алисы, которая деликатно молчала. Время полностью разладилось, разорвалось в клочья. Оно осталось только в нашей вере и любви. Зачем я здесь?]],
				[[-- Передача из центра звездных полетов от 11 апреля 2254 года -- голос Алисы деликатно вывел меня из задумчивости.^-- Да, спасибо, Алиса.]],
				...
				[[-- Таким образом по нашим наблюдениям выполнение программы идет строго по плану.]];
				...
				[[И снова синий экран.^-- Передача от "Пионера-2217" от 20 февраля 2256 года.^-- Да, спасибо.]];
...
на "Пионере", время на Земле, время на "Пилигриме" и на "Ковчеге"? Какое из них настоящее? И все-таки, оно должно быть. Настоящее.]];
			}
			if pager(s, txt) then
				walkout()
			end
