...
obj {
	nam = 'мяч';
	inv = [[Этот мяч мне подарил отец на 7 лет.]];
	dsc = [[{#лестница|Под лестницей} {лежит мой мяч.}]];
	tak = [[Я забрал мяч.]];
}

dict.add("я7", "Мне семь лет.")
dict.add("мяч", "Волейбольный мяч, который мне подарил отец на 7 лет.");
...
{$d дома|Возле соседнего дома} {$d я7|я} {#площадка|вижу площадку.}]];
	...
	way = { 'Жилой Отсек 1', path {'#к_площадке', 'К площадке', 'площадка' }:disable() };
}: with {
...
room {
	nam = 'площадка';
	title = 'Площадка';
	decor = [[{#площадка|Детская площадка пуста,} {#мальчик|если не считать мальчика, который} {#лестница|сидит на самом конце лестницы} {#смотрит|и молча смотрит куда-то наверх.}]];
	way = { path {'К подъезду', 'Двор' }, path {'#наверх', 'Наверх', 'Лестница'}:disable() };
	exit = function(s, t)
...
room {
	...
	decor = [[{$d я7|Я} {#ступенька|сижу на ступеньке}. {#макс|Рядом сидит Макс.}]];
