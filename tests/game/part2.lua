...
подбежала ко мне и быстро спросила:^-- Где здесь аптека? Там человеку плохо...]], true
	...
	end;
	{
		text = {
			...
			[[Я знаю, что у меня нет шансов, но я так же знаю, что у меня нет выбора.]],
...
room {
	nam = 'На улице';
enter = [[Все было уже подготовлено. Монета лежала там, где ее нельзя было не заметить.
...
room {
	nam = 'В кафе';
	title = 'В пивной';
	enter = [[-- Ну и что, что было дальше? -- Владимир не скрывает своего интереса, хотя
в уголках его глах я вижу скрытое лукавство.]];
		...
		obj {
			...
			pos = 1;
			{
				text = {
					...
					[[-- Ну а дальше, я пробрался на работорговца, который и вернул меня сюда...]],
