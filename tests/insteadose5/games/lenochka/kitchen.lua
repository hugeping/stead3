...
obj{
    ...
    inv=function(s,w)
	local instr = [[]];
	instr = instr..[[1- Выкинуть остатки старого кофе]];
	if drink > 0 then instr = instr..[[(x);^]] else instr = instr..[[;^]]; end ;
	instr = instr..[[2- Ополоснуть фильтр]];
	if drink > 1 then instr = instr..[[(x);^]] else instr = instr..[[;^]]; end ;
	instr = instr..[[3- Налить воды]];
	if drink > 3 then instr = instr..[[(x);^]] else instr = instr..[[;^]]; end ;
	instr = instr..[[4- Насыпать кофе]];
	if drink > 4 then instr = instr..[[(x);^]] else instr = instr..[[;^]]; end ;
	instr = instr..[[5- Включить;^]];
	instr = instr..[[6- Ждать пока кофе будет готово;]];
	if drink > 5 then instr = [[Лена снова спрятала свою "шпаргалку" в шкаф.]]; remove ("Инструкция"); end ;
	p (instr);
	end;
	}
