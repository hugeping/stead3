...
dlg { 	nam = 'выбор_платья';
	...
	phr = {
			...
			{ hidden = true, '#выбор',
				{'А как же, то синее платье?', '--Исключено. Оно не подходит к сумочке, которую я уже выбрала',
					{'Ну тогда красное.','--Оно тем более не подходит'},
					{'У меня свежая идея. Возьми другую сумку!','--Идти со старой сумкой, когда наконец можно поносить новую? Ну уж нет!'}
				},
				{'Может пойдешь как вчера?', '--Ты что, издеваешься?'},
				{'У тебя полный шкаф вещей!', '--Это НЕПОДХОДЯЩИЕ вещи.';
...
dlg { nam = 'водитель';
	...
	phr = {
			...
			{'Уже поздно, в институте будет кто-то? Я к тому, что может сразу в гостиницу?','--Не волнуйтесь, там всегда кто-то есть. И гостиница там напротив в двух шагах.'},
			onempty = function()
...
-- goo hara-gamkra vahita . hanaha vahita kuksata manahata hara rumdan tah tarjan . kir durjala vahita famhata . jags tarta vahita takja tur hara-gamkra vahita . pakra rumdan dusur durjala gamkra
...
protocol { nam = 'текст_диск',
    ...
    obj = {  
			o_trns_full(),
			--lblc('ФОТОКИОПИЯ'),
			lbl('Надпись на диске выполнена в виде сходящиейся спирали:^goo hara-gamkra vahita . hanaha vahita kuksata manahata hara rumdan tah tarjan . kir durjala vahita famhata . jags tarta vahita takja tur hara-gamkra vahita . pakra rumdan dusur durjala gamkra^^Перевод:'),
			wrdn('goo'),
			wrdn('hara'),
			obj {dsc = '-'},
			wrdn('gamkra'),			
...
protocol { 	nam = 'протокол_3';
	...
	obj = {     
				...
				wrd('ruha'); --говорить
				wrd('jurfat'); --мужчина				
				lbl('Воздействие: передан женский смех ^Ответ: ruha simg warana ruhana'); 	
				wrd('ruha'); --говорить
				wrd('simg'); --огонь				
				wrd('warana'); --женщина
				wrd('ruhana'); --рот				
...
protocol { 	nam = 'протокол_4';
	...
	obj = {     
				...
				--сохранять рука женщина много растение-ребенок  
				wrd('tarta'); 
				wrd('han');
 				wrd('warana');	
				wrd('hara');
				...
				wrd('famhata');
				wrd('jurfat');				
				lbl('Воздействие: видео, мужчина с загипсованной рукой машет женщине и смеется^Ответ: goo kir di han jurfat');  
				--быть нет один рука мужчина 
				wrd('goo');
				wrd('kir');
				wrd('di');
				wrd('han');
...
protocol { 	nam = 'идея_1';
	...
	decor =  'Надпись на диске выполнена в виде сходящиейся спирали: goo hara-gamkra vahita . hanaha vahita kuksata manahata hara rumdan tah tarjan . kir durjala vahita famhata . jags tarta vahita takja tur hara-gamkra vahita . pakra rumdan dusur durjala gamkra';
	obj = {     
...
protocol { 	nam = 'идея_2';
	...
	obj = {     
				...
				wrd('pakra'); 
				lblc('возможно оно как-то связано с полетом или птицами? Кроме этого через тот же иероглиф пишетя слово'),
				wrd('gamkra');
				lblc(', дословно "то вокруг чего летят птицы". Возможно имеется в виду некое пространство?'),
		};
}
...
protocol { 	nam = 'идея_3';
	...
	obj = {     
				...
				lblc('Судя по морфологии языка, слова со сходной флексией -hata/-sata, это прилагательные обозначающие некие качества. Таких на диске встречается три:'),
				wrd('kuksata'); 
				wrd('manahata');
 				wrd('famhata');				
...
dlg { 	nam = 'задача2';
	...
	phr = {
			...
			{ hidden = true, '#дальше',
				...
				onempty = function()
					p 'Генерал многозначительно посмотрел на меня ивышел в коридор.'
					walkout()
				end;
		}
...
dlg { 	nam = 'задача3';
	...
	phr = { 		
			...
			{ hidden = true, '#кто',
				{function() p ( 'Судя по всему в тексте они называются "'..dict['kuksata'][2]..'-'..dict['manahata'][2]..' '..dict['hara'][2]..' '..dict['rumdan'][2]..'" они пришли с каких-то гор.') end,'--Не совсем понятно. Так кто они такие?',
					only = true,
			...
			{ hidden = true, '#что2', function() p ('--Понимаете, там есть одно очень сложное для перевода место. Я не уверен, но если дословно, то в нем прячется "'..dict['tur'][2]..'". Еще там сказано: "'..dict['pakra'][2]..' '..dict['rumdan'][2]..' '..dict['dusur'][2]..' '..dict['durjala'][2]..' '..dict['gamkra'][2]..'", -- генерал нахмурился, -- Билеберда какая-то. А если по-русски?') end,
					only = true,
...
room {	nam = 'кабинет_генерала';
	...
	obj = { 
			...
			obj { nam = 'подстаканник';
				...
				act = function(s) 
							if not (where('чайник')^'#шкаф') and  where('чай')^'кабинет_генерала'  then 
								take('чай')
								take('стакан')
								return 'Товарищ генерал, я тут у себя чайник нашел. Может у вас есть стакан и чай заодно? -- генерал хмыкнул и достал откуда-то из под стола чистый граненый стакан, пакетик черного чая и протянул мне, --Спасибо!'
							else 
								return 'Красивый подстаканник, видимо какой-нибудь сувенир на 23 февраля, или что там праздную эти генералы.'  
							end;
						end; 
			...
			obj { nam = 'чай',
				...
				use = function(s,w)
						if w^'стакан' and _'стакан'.full then 
							purge(s)
							_'стакан'.tea = true
...
room { nam = 'коридор_5',
		...
		decor = function(s)
			      if progress == 0 and disabled(_'#кот') then return 'Я стою в коридоре 5-го этажа. Тут такоей же {#полумрак|полумрак} как на четвертом этаже. Коридор перегорожен белой металлической {белая_дверь|дверью} с красной {#надписью|надписью}.'
...
room {  nam = 'улица',
		...
		obj = {    'голуби',
					obj {nam = '#дождь',
							act = 'Я вдыхаю прохладный влажный воздух. Пахнет асфальтом, прелыми листьями и мокрой землей.'
							},
					obj {nam = '#посетители',
...
obj { nam = 'бутерброд',
	 disp = 'Бутерброд',
	 inv = 'Бутерброд с сыром, я купил его в небольшом продуктовом магазине по пути в инстиут. Не хочется есть его всухомятку.',
	 use = function(s,w)
...
obj { 	nam = 'мел',
		...
		use = function(s,w)
				...
				elseif 	w.isMan then p 'Не думаю, чт оему это нужно.'
