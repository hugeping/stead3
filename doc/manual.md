...
сцены (в графической интерпретаторе) с возможными переходами в другие
сцены.
...
Переменные объекта должны быть определены заранее. Примерно так, как
мы поступили с seen. Но переменных может быть много.
...
На самом деле, кроме 'game.act' и 'act' существуют обработчики 'onact'
у объектов game, игрока и текущей сцены, которые могут прервать
выполнение обработчика 'act'.

Перед тем как вызвать обработчик 'act' у объекта, вызывается onact у
game, потом у объекта-игрока, а затем -- 'onact' у текущей
комнаты. Если хотя бы один из этих обработчиков вернет false, вся
цепочка вызовов обрывается. 'onact' удобно использовать, для контроля
событий в комнате или гре, например:
...
текущую сцену;
- new(конструктор, аргументы) - создание нового _динамического_ объекта;
- delete(w) - удаление динамического объекта;
- gamefile(фалйл, [сбросить состояние?]) - подгрузить динамически файл с игрой;
...
- pop(текст, тег) - конструирует функцию, которая выводит текст и
  делает pop;
- push(тег) - переход в следующую ветвь диалога
- push(текст, тег) - конструирует функцию, которая выводит текст и
  делает push;
...
- remove(w, [wh]) - удалить объект из объекта или комнаты;
- purge(w) - уничтожить объект (из всех списков);
...
- time() - число ходов от начало игры.


__Важно!__

На самом деле, эти функции также умеют работать не только с комнатами
и объектами, но и со списками. То есть 'remove(apple, inv())'
сработает также как и 'remove(apple, me())''; Впрочем, remove(apple)
тоже сработает и удалит объект из тех мест, где он присутствует.
